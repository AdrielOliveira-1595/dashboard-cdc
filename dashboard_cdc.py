...
def processar_dados(df: pd.DataFrame) -> pd.DataFrame | None:
    ...
    def _parse_date(val):
        ...
        s = s.strip()
        # Formato brasileiro DD/MM/AA (padrão da planilha)
        ...
        if m:
            ...
            if len(partes) == 3:
                ...
                if yy < 100:
                    yy += 2000
                # Se mm > 12, os valores estão invertidos — corrige
                if mm > 12:
                    dd, mm = mm, dd
                try:
...
<div style="text-align:center;padding:20px 0 8px">
  ...
  <div style="font-size:22px;font-weight:900;letter-spacing:-0.03em">TITÃS 2</div>
